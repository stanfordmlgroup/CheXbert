...
from datasets.impressions_logits_dataset import ImpressionsLogitsDataset
from loss.ce_logits_loss import CELogitsLoss
from constants import *

def collate_fn_logits(sample_list):
     ...
     @param sample_list (List): A list of samples. Each sample is a dictionary with
                                keys 'imp', 'logits_13', 'logits_no_finding', and 'len' 
                                as returned by the __getitem__ function of ImpressionsDataset
     
     @returns batch (dictionary): A dictionary with keys 'imp', 'logits_13', 'logits_no_finding', 
                                  and 'len' but now 'imp' is a tensor with padding and batch size 
                                  as the first dimension. 'logits_13' and 'logits_no_finding' are 
                                  stacked tensors of logits for the whole batch with batch size as 
                                  first dim. And 'len' is a list of the length of each sequence 
                                  in batch
     """
     ...
     logits_13_list = [s['logits_13'] for s in sample_list]
     logits_no_finding_list = [s['logits_no_finding'] for s in sample_list]
     batched_logits_13 = torch.stack(logits_13_list, dim=0)
     batched_logits_no_finding = torch.stack(logits_no_finding_list, dim=0)
     len_list = [s['len'] for s in sample_list]

     batch = {'imp': batched_imp, 
              'logits_13': batched_logits_13,
              'logits_no_finding': batched_logits_no_finding, 
              'len': len_list}
     return batch
...
def train_from_logits(save_path, dataloaders, model=None, device=None,
                      optimizer=None, lr=LEARNING_RATE, temp=TEMP, log_every=LOG_EVERY,
                      valid_niter=VALID_NITER, best_metric=np.inf):
     """ Main training loop for the labeler based on logits
     ...
     for epoch in range(NUM_EPOCHS):
          for i, data in enumerate(train_ld, 0):
               ...
               for j in range(len(out)-1):
                    batch_loss += loss_func(out[j], logits_13[j])
               batch_loss += loss_func(out[-1], logits_no_finding)
               ...
               if (i+1) % valid_niter == 0:
                    print('\n begin validation')
                    dev_loss = utils.evaluate_logits(model, dev_ld, device, temp)
                    print("dev loss: %.3f" % (dev_loss))

                    if dev_loss < best_metric: #new best network
...
if __name__ == '__main__':
     #TRAIN ON LOGITS
     #checkpoint_path = '/data3/aihc-winter20-chexbert/bert/labeler_ckpt/radio_beamx1/model_epoch72_iter84'
     #model, optimizer, device = model_from_ckpt(bert_labeler(), checkpoint_path)
     #dataloaders = load_data('/data3/aihc-winter20-chexbert/bert/logits/radio_beamx1/master_train_logits',
     #                        '/data3/aihc-winter20-chexbert/bert/impressions_lists/train_impressions_list',
     #                        '/data3/aihc-winter20-chexbert/bert/logits/radio_beamx1/master_dev_logits',
     #                        '/data3/aihc-winter20-chexbert/bert/impressions_lists/dev_impressions_list',
     #                        self_training=True)
     #train_from_logits(save_path='/data3/aihc-winter20-chexbert/bert/labeler_ckpt/self_train/', 
     ...
     #CREATE LOGITS FILES
     #loader = load_test_data('/data3/aihc-winter20-chexbert/chexpert_data/master_dev.csv', 
     #                        '/data3/aihc-winter20-chexbert/bert/impressions_lists/dev_impressions_list')
     #ckpt_path = "/data3/aihc-winter20-chexbert/bert/labeler_ckpt/radio_beamx1/model_epoch72_iter84"
     #save_path = "/data3/aihc-winter20-chexbert/bert/logits/radio_beamx1/master_dev_logits"
     #model, optimizer, device = model_from_ckpt(bert_labeler(), ckpt_path)
     #utils.save_logits(model, loader, device, save_path)
